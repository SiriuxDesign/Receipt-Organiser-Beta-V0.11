...
function ReceiptHistory({ receipts, updateReceipt, deleteReceipt }) {
  ...
  const handleDeleteClick = (index) => {
    ...
    if (window.confirm('Are you sure you want to delete this receipt?')) {
      ...
      // Adjust expandedReceipts indices after deletion if necessary
      setExpandedReceipts(prevExpanded =>
        prevExpanded
          .filter(i => i !== index) // Remove the deleted index
          .map(i => (i > index ? i - 1 : i)) // Decrement indices after the deleted one
      );
    }
  }
  ...
  const filteredAndSortedReceipts = useMemo(() => {
    ...
    let filteredReceiptsWithFilteredItems = receipts
      .map(receipt => {
        // Filter items within the receipt
        const filteredItems = receipt.items.filter(item => {
          const itemMatch = searchTermLower === '' ||
          ...
          return itemMatch && typeMatch && categoryMatch;
        });

        // If any items match, include the receipt with the filtered items
        if (filteredItems.length > 0) {
          ...
          // Also check if merchant or notes match the search term if no items matched
          ...
          if ((yearMatch && financialYearMatch) && (filteredItems.length > 0 || merchantOrNotesMatch)) {
             // Return a new object with filtered items
            return {
              ...receipt,
              items: filteredItems,
              // Recalculate total amount based on filtered items if needed,
              // or keep original total if filtering is just for display
              // Keeping original total for now as per previous logic
              // totalAmount: filteredItems.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0)
            };
          }
        }
        return null; // Exclude receipt if no items match filters/search and merchant/notes don't match search
      })
      .filter(receipt => receipt !== null); // Remove null entries

    // Apply sorting to the filtered list of receipts
    filteredReceiptsWithFilteredItems.sort((a, b) => {
      let comparison = 0;
      if (sortBy === 'date') {
    ...
    return filteredReceiptsWithFilteredItems;
